...
{
        $tpl_source = "~~mod mod_name='get_template' action='".$tpl_name."'~";
        return true;
}
...
{
    $result = rows("SELECT `content` FROM `tree` WHERE `name` = ".$tpl_name);
    $tpl_source=$result[0]['content'];
    return true;
}

function db_get_timestamp($tpl_name, &$tpl_timestamp, &$smarty_obj)
{
    $result = rows("SELECT UNIX_TIMESTAMP() as tpl_timestamp");
    $tpl_timestamp=$result[0]['tpl_timestamp'];
    return true;
}
...
function db2_get_template ($tpl_name, &$tpl_source, &$smarty_obj)
{
    $result = rows("SELECT `content` FROM `templates` WHERE `id` = ".$tpl_name);
    $tpl_source=$result[0]['content'];
    return true;
}

function db2_get_timestamp($tpl_name, &$tpl_timestamp, &$smarty_obj)
{
    $result = rows("SELECT UNIX_TIMESTAMP() as tpl_timestamp");
    $tpl_timestamp=$result[0]['tpl_timestamp'];
    return true;
...
function db2_get_secure($tpl_name, &$smarty_obj)
{    // предполагаем, что наши шаблоны совершенно безопасны
    return true;
}

function db2_get_trusted($tpl_name, &$smarty_obj)
{    // не используется для шаблонов
}    
