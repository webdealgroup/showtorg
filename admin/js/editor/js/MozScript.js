...
function wp_context(obj, evnt) {
	...
	if (imageNode) {
		if ((imageNode.getAttribute('name')) && (imageNode.src.search(wp_directory+"images/bookmark_symbol.gif") != -1)) {
			var menu = document.getElementById(obj.id+"_bookmarkMenu")
			oWidth=230
			oHeight = obj.bmenu_height + 2
		} else {
			var menu = document.getElementById(obj.id+"_imageMenu")
			oWidth=230
			oHeight = obj.imenu_height + 2
		}
	} else if (wp_isInside(obj, 'TD')) {
		var menu = document.getElementById(obj.id+"_tableMenu")
		oWidth=270
		oHeight = obj.tmenu_height + 2
		wp_getTable(obj)
	} else {
		var menu = document.getElementById(obj.id+"_standardMenu")
		oWidth=230
	...
	if (n >= 1) {
		...
		var frame = document.getElementById(obj.id+'_editFrame')
		...
		setTimeout("document.addEventListener('mouseup', wp_closePopupTimer, true)", 200);
	}
...
function wp_closePopup(objname) {
	...
	try {document.removeEventListener('mouseup', wp_closePopupTimer, true)} catch (e) {}
}
...
function wp_divReturn(obj, evt) {
	...
	var scrollBottom = document.getElementById(obj.id+'_editFrame').style.height
...
function wp_editor(obj,config) {
	...
	// objects:
	obj.html_edit_area = document.getElementById(obj.id)
	obj.format_list=document.getElementById(obj.id+'_format_list')
	obj.font_face=document.getElementById(obj.id+'_font_face')
	obj.font_size=document.getElementById(obj.id+'_font_size')
	obj.class_menu=document.getElementById(obj.id+'_class_menu')
	obj.foo = obj.html_edit_area.value
	// 222 frame position fix
	obj.format_frame = document.getElementById(obj.id+"_format_frame").contentWindow
	obj.class_frame = document.getElementById(obj.id+"_class_frame").contentWindow
	obj.font_frame = document.getElementById(obj.id+"_font_frame").contentWindow
	obj.size_frame = document.getElementById(obj.id+"_size_frame").contentWindow
	try {
	...
	// ens 222
	var tbar=eval("document.getElementById('"+obj.id+"_tab_one')")
	var tbarimages = document.getElementById(obj.id+"_tab_one").getElementsByTagName('IMG')
	obj.tbarimages = tbarimages
	obj.tbarlength = tbarimages.length
	obj.safe = true
	obj.edit_object = document.getElementById(obj.id+'_editFrame').contentWindow
	obj.editFrame = obj.edit_object
	obj.previewFrame = document.getElementById(obj.id+"_previewFrame").contentWindow
	// submit_form
...
function wp_send_to_edit_object(obj, init) {
	...
	// wp 222
	eval(obj.id+".edit_object.document.addEventListener('mouseup', wp_"+obj.id+"_mouseUpHandler, true)")
	eval(obj.id+".edit_object.document.addEventListener('mousedown', wp_closePopup, true)")
	eval(obj.id+".edit_object.document.addEventListener('keypress', wp_"+obj.id+"_keyHandler, true)")
	eval(obj.id+".edit_object.document.addEventListener('contextmenu', wp_"+obj.id+"_contextHandler, true)")
	if (init) {
...
function wp_create_image_html(obj, iurl, iwidth, iheight, ialign, ialt, iborder, imargin) {
	...
	if (imageNode) {
		...
		var img = obj.edit_object.document.createElement("img")
	}
...
function wp_set_button_states(obj) {
	try {
		obj.edit_object.document.queryCommandValue('FontName')
	} catch (e) {
		wp_reactivate(obj.name);
	}
	obj.initfocus = true
	...
	var class_menu_text = document.getElementById(obj.id+'_class_menu-text')
	if (class_menu_value) {
...
function wp_set_list_text (list, value, lang, obj) {
	var list_text = document.getElementById(obj.id+'_'+list+'-text')
	if (value) {
