...
function wp_editor(obj,config) {
	...
	obj.format_frame = document.frames(obj.id+"_format_frame")
	obj.class_frame = document.frames(obj.id+"_class_frame")
	obj.font_frame = document.frames(obj.id+"_font_frame")
	obj.size_frame = document.frames(obj.id+"_size_frame")
	obj.format_frame.written = false
	obj.class_frame.written = false
	obj.font_frame.written = false
	obj.size_frame.written = false
	// end 222
	obj.font_face = eval("document.all."+obj.id+"_font_face")
	obj.font_size = eval("document.all."+obj.id+"_font_size")
	obj.format_list = eval("document.all."+obj.id+"_format_list")
	obj.class_menu = eval("document.all."+obj.id+"_class_menu")
	obj.html_edit_area = eval("document.getElementById('"+obj.id+"')")
	...
	obj.edit_object = document.frames(obj.id+"_editFrame")
	obj.editFrame = obj.edit_object
	obj.previewFrame = document.frames(obj.id+"_previewFrame")
	// submit_form
	...
	// end
	if (!wp_is_ie50) {
		var table = document.getElementById(obj.id+"_toolbar1")
		var n = table.all.length
		for (i=0; i<n; i++) {
			table.all[i].unselectable = "on"
		}
		var table = document.getElementById(obj.id+"_toolbar2")
		var n = table.all.length
		for (i=0; i<n; i++) {
			table.all[i].unselectable = "on"
		}
		var tab_table = document.getElementById(obj.id+'_tab_table')
		var n=tab_table.all.length
		for (i=0; i<n; i++) {
			tab_table.all[i].unselectable = "on"
		}
		var hidden = document.getElementById(obj.id+'_hidden')
...
function wp_next (obj) {
	...
	if (!wp_is_ie50) {
		...
		obj.edit_object.document.oncontextmenu = function () {
			if (obj.safe) {
				...
				if (menu == document.getElementById(obj.id+"_tableMenu")) {
					wp_getTable(obj, sel)
				}
	...
	if (document.attachEvent) {
		eval(obj.id+".edit_object.document.attachEvent('onmouseup', wp_"+obj.id+"_mouseUpHandler, true)")
	} else {
		eval(obj.id+".edit_object.document.onmouseup = wp_"+obj.id+"_mouseUpHandler")
	}
...
function wp_create_image_html(obj,iurl, iwidth, iheight, ialign, ialt, iborder, imargin) {
	...
	if (obj.edit_object.document.selection.type == "Control") {
		var selOld = obj.edit_object.document.selection.createRange()
		if (selOld(0).tagName == "IMG") {
			var attributes = selOld(0).attributes;
		}
	}
	if (document.getElementById(obj.id+'_class_menu-text').innerHTML != "Class")
		document.getElementById(obj.id+'_class_menu-text').innerHTML = "Class"
	var sel = obj.edit_object.document.selection.createRange()
	...
	if (obj.edit_object.document.selection.type == "Control") {
		...
		if (sel(0).tagName == "IMG") {
			// are we updating an existing image?
			if (attributes) {
				wp_add_attributes(sel(0), attributes);
				sel(0).setAttribute('src', iurl);
			}			
