...
class wysiwygPro {
	...
	*/
	...
	/***************************************************************************
	 loadmethod
	 Public: specifies whether the editor should load inline or after the document has finished loading.
	 $method: "onload" means it will load after the document has loaded. "inline" means it will load while the document is still loading.
	...
	*/	
	...
	function loadmethod($method='onload') {
		if ($this->has_expired) {
			if ($method == 'inline') {
				$this->loadmethod = 'inline';
			} else {
				$this->loadmethod = 'onload';
			}
		}
	}
	...
	function set_baseurl($baseURL='') {
		if ($this->has_expired) {
			if (!empty($baseURL)) {
				...
				$this->domain = str_replace('/','\/',quotemeta( preg_replace('/(^http(|s):\/\/[^\/]+).*/smi', "$1", $baseURL) ) );
			}
		}
	}
	...
	*/
	...
	function addbutton($title, $location, $function, $url, $width=NULL, $height=NULL, $cid='forecolor') {
		if ($this->has_expired) {
			$button = "<td><img cid=\"$cid\" class=\"wpReady\" onclick=\"$function\" src=\"$url\"";
			if (is_int($width)) {
				$button .= ' width="'.$width.'"';
			}
			if (is_int($height)) {
			...
			if (!is_array($links)) {
				...
				for ($i=0; $i<$num_links; $i++) { 
					if ((!empty($links[$i][1])) && (!empty($links[$i][2]))) {
						array_push($arr, '['.intval($links[$i][0]).',"'.str_replace(array('"', '/'), array('\"', '\/'), $links[$i][1]).'","'.str_replace(array('"', '/'), array('\"', '\/'), $links[$i][2]).'"]');
					}	
				}
	...
	*/
	...
	function set_savebutton($name, $url=NULL, $width=NULL, $height=NULL) {
		if ($this->has_expired) {
			...
			$save_button = "\n<!-- begin save -->\n<td><input cid=\"ignore\" style=\"cursor:default\" class=\"wpReady\" id=\"##name##_wp_save\" name=\"##name##_wp_save\" type=\"image\" src=\"$url\"";
			if (is_int($width)) {
	...
	*/ 
	...
	function return_editor($width=680, $height=390) {
		...
		if ($this->has_expired) {
			...
			// load method...
			if ($this->loadmethod == 'onload') {
				$loader = '
<!-- begin 1st instance only -->
<script language="JavaScript" type="text/javascript" src="##directory##js/loader.js"></script>
<!-- end 1st instance only -->
';
			} else {
				$loader = '<script language="JavaScript" type="text/javascript">wp_editor(##name##,config_##name##);</script>';
			}
			...
			$this->WYSIWYGPRO_code = $this->wp_replace($this->WYSIWYGPRO_code, array(  
				...
				'domain' => empty($this->domain) ? $this->domain : $this->domain."[^\/]+",
