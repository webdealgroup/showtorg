...
if (!empty($_POST['editnode'])) {
    ...
    $row = row("SELECT COUNT(*) as count FROM description WHERE id='".$id."'");
    
    //echo "<pre>"; print_r($row); echo "</pre>"; //die();
		if (!$row['count']) {
            insert('description', array('id' => $id, 'description' => str_replace($order, $replace, $_REQUEST['description'])));
            //sleep(1);
		} else {
            update('description', array('id' => $id), array('description' => str_replace($order, $replace, $_REQUEST['description'])));
		}
	$saved = true;
}
...
$data = row("SELECT * FROM description WHERE id=".$id);
...
$textarea = $editor->return_editor('100%', '433');
...
#content_container iframe { height: 80%; }
