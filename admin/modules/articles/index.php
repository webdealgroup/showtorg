...
function updateSort($order) {
	...
	$i = 0;
	foreach ($order as $id) {
		update('articles', array('id' => $id), array('sort' => $i));
		$i++;
	}
}
...
case "editURL":
	...
	$name = $_POST['name'];
	if ($id AND $parent_id) {
		$row = row("SELECT COUNT(*) as count FROM articles WHERE name='".mysql_escape_string($name)."' AND parent_id='".$parent_id."'");
		if ($row['count']) {
			echo json(array('message' => 'Страница с таким URL уже есть в этом каталоге!'));
		} else {
			update('articles', array('id' => $id), array('name' => $name));
...
case "editNode":
	...
	update('articles', array('id' => $node_id), array(
		'name' => $node_id,
		'parent_id' => $parent_id
	));
...
case "saveOrder":
	$node_id = intval($_POST['node_id']);
	$parent_id = intval($_POST['parent_id']);
	update('articles', array('id' => $node_id), array('parent_id' => $parent_id));
	updateSort($_POST['order']);
...
//	Если в родительском каталоге появилось более одной страницы с одинаковым name, переименовываем перемещённую страницу и выводим сообщение
	$row = row("SELECT name FROM articles WHERE id='".$node_id."'");
	$check = row("SELECT COUNT(*) as count FROM articles WHERE name='".esc($row['name'])."'");
	if ($check['count'] > 1) {
		$newname = time();
		update('articles', array('id' => $node_id), array('name' => $newname));
...
case "getNode":
	...
	$nodes = rows("SELECT id, name, title as text, parent_id, tpl, status FROM articles WHERE parent_id='".$node."' ORDER BY sort ASC");
	foreach ($nodes as $k=>$v) {
		list($children) = row("SELECT COUNT(*) as count FROM articles WHERE parent_id='".$v['id']."'", 'array');
