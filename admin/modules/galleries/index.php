...
function updateSort($order) {
	...
	$i = 0;
	foreach ($order as $id) {
		update('galleries', array('id' => $id), array('sort' => $i));
		$i++;
	}
}
...
case "editURL":
	...
	$name = $_POST['name'];
	if ($id AND $parent_id) {
		$row = row("SELECT COUNT(*) as count FROM galleries WHERE name='".mysql_escape_string($name)."' AND parent_id='".$parent_id."'");
		if ($row['count']) {
			echo json(array('message' => 'РЎС‚СЂР°РЅРёС†Р° СЃ С‚Р°РєРёРј URL СѓР¶Рµ РµСЃС‚СЊ РІ СЌС‚РѕРј РєР°С‚Р°Р»РѕРіРµ!'));
		} else {
			update('galleries', array('id' => $id), array('name' => $name));
...
case "editTitle":
	...
	$name = $_POST['name'];
	if ($id AND $parent_id) {
		$row = row("SELECT COUNT(*) as count FROM galleries WHERE title='".mysql_escape_string($name)."' AND parent_id='".$parent_id."'");
		if ($row['count']) {
			echo json(array('message' => 'РЎС‚СЂР°РЅРёС†Р° СЃ С‚Р°РєРёРј URL СѓР¶Рµ РµСЃС‚СЊ РІ СЌС‚РѕРј РєР°С‚Р°Р»РѕРіРµ!'));
		} else {
			update('galleries', array('id' => $id), array('title' => $name));
...
case "editNode":
	...
	update('galleries', array('id' => $node_id), array(
		'name' => $node_id,
		'parent_id' => $parent_id
	));
...
case "saveOrder":
	$node_id = intval($_POST['node_id']);
	$parent_id = intval($_POST['parent_id']);
	update('galleries', array('id' => $node_id), array('parent_id' => $parent_id));
	updateSort($_POST['order']);
...
//	Р•СЃР»Рё РІ СЂРѕРґРёС‚РµР»СЊСЃРєРѕРј РєР°С‚Р°Р»РѕРіРµ РїРѕСЏРІРёР»РѕСЃСЊ Р±РѕР»РµРµ РѕРґРЅРѕР№ СЃС‚СЂР°РЅРёС†С‹ СЃ РѕРґРёРЅР°РєРѕРІС‹Рј name, РїРµСЂРµРёРјРµРЅРѕРІС‹РІР°РµРј РїРµСЂРµРјРµС‰С‘РЅРЅСѓСЋ СЃС‚СЂР°РЅРёС†Сѓ Рё РІС‹РІРѕРґРёРј СЃРѕРѕР±С‰РµРЅРёРµ
	$row = row("SELECT name FROM galleries WHERE id='".$node_id."'");
	$check = row("SELECT COUNT(*) as count FROM galleries WHERE name='".esc($row['name'])."'");
	if ($check['count'] > 1) {
		$newname = time();
		update('galleries', array('id' => $node_id), array('name' => $newname));
...
case "getNode":
	...
	$nodes = rows("SELECT id, name, title as text, parent_id, tpl, status FROM galleries WHERE parent_id='".$node."' ORDER BY sort ASC");//AND status = 2 ORDER BY sort ASC");
    
    
	foreach ($nodes as $k=>$v) {
		list($children) = row("SELECT COUNT(*) as count FROM galleries WHERE parent_id='".$v['id']."'", 'array');
