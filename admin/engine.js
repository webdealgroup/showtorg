Ext.BLANK_IMAGE_URL = '/desktop/images/s.gif';
...
MyDesktop = new Ext.app.App({
    ...
    getModules: function(){
        return [new MyDesktop.Windows.tree(), new MyDesktop.Windows.pages(), new MyDesktop.Windows.galleries(), new MyDesktop.Windows.templates(), new MyDesktop.Windows.modules()];
    },
    ...
    getStartConfig: function(){
        return {
            ...
            toolItems: [{
                ...
                iconCls: 'settings',
                handler: function(){
                    this.getModule('settings-win').createWindow();
                },
                scope: this
            }, '-', {
                text: 'Выход',
...
MyDesktop.Windows.tree = Ext.extend(Ext.app.Module, new function(){
    ...
    this.getWindowSettings = function(){
        ...
        var editShortDescription = function(){
            ...
            if (!node) 
                return Ext.MessageBox.alert('Ошибка', 'Выделите товар для редактирования.');
            var win;
            if (!win) {
                win = new Ext.Window({
                    applyTo: null,
        ...
        var grid = new Ext.grid.EditorGridPanel({
            ...
            tbar: [{
                ...
                handler: function(){
                    ...
                    grid.getStore().load({
                        params: {
                            node: my_tree.getSelectionModel().getSelectedNode().id,
                            start: 0,
                            limit: 100
                        }
                    });
        ...
        var tpl = new Ext.XTemplate('<tpl for=".">', '<div class="thumb-wrap" id="{name}">', '<div class="thumb"><img src="../get_image.php?file=/{url}&width=100&height=100"></div>', '<span class="x-noneditable">{shortName}</span></div>', '</tpl>', '<div class="x-clear"></div>');
        ...
        var bottomButton = new Ext.Toolbar.Button({
            ...
            handler: function(){
                window.open(this.siteURL + bottomText);
            },
            scope: this
        });
        ...
        var my_tree = new Ext.tree.TreePanel({
            ...
            tbar: [{
                ...
                handler: function(){
                    var node = my_tree.root.appendChild(new Ext.tree.TreeNode({
                        allowDrag: true
                    }));
        ...
        this.addEditTab = function(node){
            if (!node) 
                return Ext.MessageBox.alert('Ошибка', 'Выделите страницу для редактирования.');
            var win;
            if (!win) {
                win = new Ext.Window({
                    applyTo: null,
        ...
        this.addUpload = function(node){
            ...
            win.on('hide', function(){
                ...
                gallery_store.load({
                    params: {
                        dir: this.getPath(my_tree.getSelectionModel().getSelectedNode())
                    }
                });
            }, this);
        };
...
MyDesktop.Windows.pages = Ext.extend(Ext.app.Module, new function(){
    ...
    this.addTab = function(data){
        ...
        this.tabs[n].on('activate', function(p){
    ...
    this.getWindowSettings = function(){
        ...
        this.tabs[this.z] = new Ext.tree.TreePanel({
            ...
            tbar: [{
                text: 'Редактировать',
                handler: function(){
                    this.addEditTab(this.tabs[this.z].getSelectionModel().getSelectedNode());
                },
                scope: this,
                iconCls: 'add'
            }, {
                ...
                text: 'Свернуть все',
                handler: function(){
                    this.tabs[this.z].collapseAll();
                    this.tabs[0].getRootNode().expand();
                },
                scope: this,
                iconCls: 'folder'
            }]
        ...
        this.tabs[this.z].on('contextmenu', function(node, event){
            ...
            var templ = [];
            var templates = ["index.tpl", "main.tpl", "page.tpl", "goods.tpl", "gallery.tpl", "galleries_index.tpl", "ajax.tpl"];
...
MyDesktop.Windows.galleries = Ext.extend(Ext.app.Module, new function(){
    ...
    this.getWindowSettings = function(){
        ...
        var tpl = new Ext.XTemplate('<tpl for=".">', '<div class="thumb-wrap" id="{name}">', '<div class="thumb"><img src="../get_image.php?file=/{url}&height=70"></div>', '<span class="x-noneditable">{shortName}</span></div>', '</tpl>', '<div class="x-clear"></div>');
        var gallery = new Ext.DataView({
            store: gallery_store,
            tpl: tpl,
            //autoHeight:true,
        ...
        var editNode = function(event){
            ...
            gallery_store.load({
                params: {
                    dir: "galleries/" + this.getPath(my_tree.getSelectionModel().getSelectedNode())
                }
            });
        };
...
// new link methods
function path(data){
    MyDesktop.activeModule().loadPage(data);
    return false;
}

function newtab(data, name){
    MyDesktop.activeModule().addTab(data);
    return false;
}

// old link methods
function content(text, name){
    MyDesktop.activeModule().setContent(text);
    return false;
}

function openwindow(data){
    MyDesktop.Windows[name].newWindow(data);
    return false;
}
